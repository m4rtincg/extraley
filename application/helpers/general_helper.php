...
function nombrenumeroclausula1($numero){
    $unidades = array(
        1=>"PRIMERO",
        2=>"SEGUNDO",
        3=>"TERCERO",
    ...
    if($numero<=0){
        ...
        if($numero<10){
            ...
            $deaux = ($de==0)?"":$decimas[$de];
            $unaux = ($un==0)?"":$unidades[$un];

            $texto = ($un==0)? $decimas[$de] : $decimas[$de]." ".$unidades[$un];
        }
    }
    return $texto;
}
...
function IsLoginUser(){
    return ($this->session->userdata('rol')==1);
}
function IsLoginGerente(){
    return ($this->session->userdata('rol')==2);
}
function IsLoginAdmin(){
    return ($this->session->userdata('rol')==3);
}
